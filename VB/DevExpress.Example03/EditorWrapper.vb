...
Namespace DevExpress.Example03
    ...
    Public Class EditorWrapper
        ...
        Public Shared ReadOnly RowHandleProperty As DependencyProperty = DependencyProperty.Register("RowHandle", GetType(Integer), GetType(EditorWrapper), New PropertyMetadata(DataControlBase.InvalidRowHandle, AddressOf OnRowHandleChanged))
        ...
        Public Property Column As GridColumn
            Get
                Return CType(GetValue(ColumnProperty), GridColumn)
            End Get
        ...
        Public Property RowHandle As Integer
            Get
                Return CInt(GetValue(RowHandleProperty))
            End Get
        ...
        Private Sub OnColumnChanged()
            ...
            Editor.SetBinding(BaseEdit.ShowBorderProperty, New Binding("IsKeyboardFocusWithin") With {.RelativeSource = New RelativeSource With {.Mode = RelativeSourceMode.Self}})
        ...
        Private Sub OnRowHandleChanged()
            If Editor IsNot Nothing Then Editor.EditValue = Nothing
        End Sub

        Private Sub Editor_PreviewKeyDown(ByVal sender As Object, ByVal e As Input.KeyEventArgs)
            If e.Key = Input.Key.Enter Then PostValue()
        End Sub

        Private Sub Editor_LostKeyboardFocus(ByVal sender As Object, ByVal e As Input.KeyboardFocusChangedEventArgs)
            PostValue()
        End Sub

        Private Sub PostValue()
            If Editor.EditValue Is Nothing Then Return
